...
SELECT st_meta6.meta_value as booked_id ,st_meta2.meta_value as check_in,st_meta3.meta_value as check_out
        ...
        function _get_where_query($where){
            ...
            global $wpdb;
            if($st_google_location_pickup = STInput::request('st_google_location_pickup', '')){
                $st_locality = sanitize_title(STInput::request('st_locality_up', ''));
                $st_sublocality_level_1 = sanitize_title(STInput::request('st_sub_up', ''));
                $st_administrative_area_level_1 = sanitize_title(STInput::request('st_admin_area_up', ''));
                $st_country = sanitize_title(STInput::request('st_country_up', ''));
                if(TravelHelper::isset_table('st_glocation')){
                    ...
                    $where .= " AND {$wpdb->prefix}posts.ID IN (SELECT post_id FROM {$wpdb->prefix}st_glocation WHERE 1=1 {$clause} AND post_type = 'st_cars')";
                }
            }
