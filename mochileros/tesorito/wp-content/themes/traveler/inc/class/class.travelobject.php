    ...
    */
	...
	static function check_cancel_able($order_id)
	{
		...
		if(!$check_order){
			...
			if(get_post_meta($item_id,'st_allow_cancel',true)=='on') {
				...
				$days=get_post_meta($item_id,'st_cancel_number_days',true);
				if($days){
					if(STDate::date_diff(strtotime('today'),strtotime($check_order->check_in))>$days){
    ...
    function st_custom_menu_mobile_item ( $items, $args ) {
        if ($args->theme_location == 'primary') {
            ...
            $html_login = st()->load_template('menu/mobile/login_select');
            $html_language = st()->load_template('menu/mobile/language_select');
            $html_currency = st()->load_template('menu/mobile/currency_select');
            $items .= $html_login.$html_currency.$html_language;
