            ...
            function _get_where_query($where)
            {
                ...
                if($st_google_location = STInput::request('st_google_location', '')){
                    ...
                    if(TravelHelper::isset_table('st_glocation')){
                        ...
                        $where .= " AND {$wpdb->prefix}posts.ID IN (SELECT post_id FROM {$wpdb->prefix}st_glocation WHERE 1=1 {$clause} AND post_type = 'st_tours')";
                    }
                }
            ...
            */
            ...
            function do_add_to_cart(){
                ...
                if(!$check_in || !$check_out){
                    STTemplate::set_message(__( 'Select a tour in the calendar.' , ST_TEXTDOMAIN ) , 'danger' );
                    $pass_validate = FALSE;
                    return false;
                }
            ...
            static function get_price_html($post_id = false,$get = false,$st_mid='',$class='')
            {
                ...
                if(isset($prices['adult']) and $prices['adult'] and $prices['discount'])
                {
                    if($show_price_free=='on' or $adult_new_html) {
                        ...
                        $html.=sprintf(__('%s %s',ST_TEXTDOMAIN),$adult_html,$adult_new_html);
                    }

                }elseif(!empty($prices['adult_new'])){
                    if($show_price_free=='on' or $adult_new_html) {
                        $html.=sprintf(__('%s',ST_TEXTDOMAIN),$adult_new_html);
                    }
                }
